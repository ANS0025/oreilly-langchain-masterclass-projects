  ...
  if submit_button:
    if job_description and uploaded_resumes:
      try:
        ...
        similar_docs = retrieve_relevant_docs(job_description, num_resumes, vector_store)
        
        for item in range(len(similar_docs)):
          with st.expander(f"{ similar_docs[item][0].metadata['source'] } (Match Score: { similar_docs[item][1] })"):
